...
annual_litter <- litter_data_raw %>% 
  ...
  summarise(daily_litter = sum(Value)) %>% 
# compute cumulative litter mass for the year for each plot and trap
  arrange(lubridate::yday(Date_collected)) %>% 
  group_by(Plot, Trap) %>% 
  mutate(cum_litter = cumsum(daily_litter)) %>%
  # compute the mean and s.d. across all traps within a plot, for each day
  group_by(Plot, Date_collected) %>% 
  summarise(mean_litter = mean(cum_litter), sd_mean_litter = sd(cum_litter))

# Line graph
avg_litter_plot <- annual_litter %>% 
  ggplot(aes(lubridate::yday(Date_collected), mean_litter, color = Plot)) +
  geom_line(size = 1.25) +
  scale_color_manual(breaks = c("LSLE","LSME","LSHE","MSLE","MSME","MSHE", "HSLE","HSME","HSHE"), 
                     values = c("LSLE" = "#969B4B", 
                 "LSME" = "#B4B979", 
                 "LSHE" = "#A89F8E", 
                 "MSLE" = "#7887A4", 
                 "MSME" = "#A8CDEC", 
...
group_by(Plot, Species_code) %>% 
summarise(lai_plot = round(mean(LAI, na.rm = TRUE), 2)) %>% 
