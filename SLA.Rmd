...
* LAI computations seem duplicated and weird
...
lai <- sla_litter_joined %>% 
  ...
  group_by(Plot, Trap) %>% 
  # In 467, we are removing NAs from lack of certain species in some plots
  # in the future we want to use the average values to fill these in
  # Maybe we do this above, in 454-457, just by using overall mean values instead of a map
  summarise(lai_trap = sum(LAI, na.rm = TRUE), .groups = "drop_last") %>% 
  summarise(lai_plot = mean(lai_trap), 
            lai_plot_sd = sd(lai_trap)) %>% 
  arrange(Plot)
print(lai)
```
...
sla_litter_joined %>% 
  ...
  group_by(Plot, Species_code) %>% 
  summarise(value = mean(LAI, na.rm = TRUE),
            .groups = "drop") ->
  lai_plot_data
