...
m_species <- aov(specific_leaf_area ~ Species_code * Position, data = sla)
...
hsd_species <- HSD.test(aov(specific_leaf_area ~ effect, data = sla),
                        "effect", group = TRUE, unbalanced = TRUE, console = TRUE)

# Make a nice plot
sla %>%
  group_by(Species_code, Position) %>% 
  summarise(sd_sla = sd(specific_leaf_area), .groups = "drop") ->
  xx

hsd_species$groups %>% 
  mutate(species = rownames(hsd_species$groups)) %>% 
  separate(species, into = c("Species_code", "Position"), sep = "\\.", remove = FALSE) %>% 
  mutate(species = forcats::fct_reorder(species, specific_leaf_area)) %>% 
  left_join(xx, by = c("Species_code", "Position")) ->
  species_position

species_position %>% 
  ggplot(aes(species, specific_leaf_area, fill = Position)) + geom_col() +
  geom_text(aes(label = groups), vjust = -1) + 
  geom_errorbar(aes(ymin = specific_leaf_area - sd_sla, ymax = specific_leaf_area + sd_sla), alpha = 0.25) +
...
In a linear model with species and canopy position as interacting terms predicting SLA, both terms
were highly significant: species (F[1,`r msp$Species_code$df`] = `r msp$Species_code$statistic`, 
`r prettyp(msp$Species_code$p.value)`) and position (F[1,`r msp$Position$df`] = `r msp$Position$statistic`, 
`r prettyp(msp$Position$p.value)`); their interaction was not (F[1,`r msp$Interaction$df`] =
`r msp$Interaction$statistic`, `r prettyp(msp$Interaction$p.value)`). A Tukey's HSD test categorized
the SLA values in this model into five groups, summarized visually in **Figure 3**. Because 
high- and low-canopy samples were frequently taken from common trees, we also used a paired T-test to
examine the effect of canopy position. This was also highly significant (T[`r tt_position$parameter`] = 
`r round(tt_position$statistic, 3)`, `r prettyp(tt_position$p.value)`), indicating, as expected, that
high-position (sun) leaves tended to have significantly lower SLA than low (shade) leaves.
...
m_elevation <- aov(specific_leaf_area ~ Species_code * Elevation, data = sla)
...
sla %>%
  ...
  group_by(Species) %>% 
  do(mod = lm(specific_leaf_area ~ Elevation, data = .)) %>% 
  tidy(mod) %>% 
...
significant (0.01 < P < 0.05) for _Nyssa sylvatica_ and _Acer rubrum_, marginally significant
(`r prettyp(mel_sp$FAGR$p.value)`) for _Fagus grandifolia_, and not significant for 
_Liquidambar styraciflua_ (`r prettyp(mel_sp$LIST$p.value)`).
...
ggplot(filter(sla_sal, Elevation == "Shoreline"), aes(Salinity, specific_leaf_area)) +
  geom_boxplot() +
  facet_wrap(~Species) +
  ylab(expression(SLA~(cm^2~g^-1))) +
  theme(strip.text = element_text(face = "italic"))
...
# Does SLA differ between elevations after accounting for species?
m_salinity <- aov(specific_leaf_area ~ Species_code * Salinity, data = sla_sal)
...
sla_sal %>%
  ...
  group_by(Species) %>% 
  do(mod = lm(specific_leaf_area ~ Salinity2, data = .)) %>% 
  tidy(mod) %>% 
  filter(term != "(Intercept)") %>% 
  left_join(species_codes, by = "Species") ->
  msa_sp
...
In an overall analysis of variance, this salinity effect was highly significant after accounting for species 
...
Litter production peaked during the autumn in these northern temperate sites (**Figure 6**). Mean
annual production was `r round(mean(al_plot$alm), 1)` g m^-2^, with an average spatial variability
(i.e. between litter traps within a plot) of `r round(mean(al_plot$alm_sd), 1)` g m^-2^. These values 
were consistent with data from the nearby NEON tower (**Figure 6**). Across all plots, 
_Fagus grandifolia_ dominated, comprising `r round(al_species$FAGR$frac * 100, 0)`% of total mass.
...
Overall leaf area index (LAI, m2 m^-2^) inferred from the combination of SLA and litterfall data
ranged from a value of `r min(mc_lai_plot$LAI)` 
...
"North Branch" site). The low-elevation plots' LAI values were inversely related to their salinity
exposure, with values from `r mc_lai_sal$HSLE$LAI` `r mc_lai_sal$HSLE$ci` to 
`r mc_lai_sal$MSLE$LAI` `r mc_lai_sal$MSLE$ci` to `r mc_lai_sal$LSLE$LAI` `r mc_lai_sal$LSLE$ci` 
at the lowest-salinity plot. 
Spatial variability in leaf litter production contributed much more to the variability in LAI estimates
than did variability in SLA measurements (**Figure 7**).
