...
map <- ggmap(sites_map) +
  ...
  scale_x_continuous(label  s = scales::number_format(accuracy = 0.01), expand = c(0,0))
# +
  # annotate(geom = "text", x = -76.565, y = 38.88, label = "Smithsonian Environmental\nResearch Center", size = 3)
print(map)
ggsave("poster_figures/map.png")
```
...
```{r read-sla}
## ----- Read in the SLA csv -----
sla_raw <- read_csv("SLA Data.csv", col_types = "ccccdcdddc")
...
ss_inventory <- read_csv("ss-inventory.csv", col_types = "c-cd-c----")
...
transplant_inventory <- read_csv("inventory.csv", col_types = "-cc-c---d--c") %>% 
...
if(any(duplicated(transplant_inventory$Tag))) {
  ...
  paste("Index:", which(duplicated(transplant_inventory$Tag)))
}

# Combine Storm Surge inventory with Transplant inventory
inventory <- bind_rows(transplant_inventory, ss_inventory) %>% 
  filter(!is.na(Tag))
...
df <- tibble(Plot = shoreline_plots, 
...
sla_shoreline %>%
  group_by(Species_common, Salinity) %>% 
  summarise(mean_SLA = paste(round(mean(specific_leaf_area), digits = 2), "+/-",
                             round(sd(specific_leaf_area), digits = 2))) %>% 
...
```
### Poster Figures
``` {r Poster Figures, echo=FALSE}
# American Beech is too long! Shorten to Beech
sla_poster <- sla %>% 
  mutate(Species_common = if_else(Species_common == "American beech", "Beech", Species_common))
sla_shore_poster <- sla_shoreline %>% 
   mutate(Species_common = if_else(Species_common == "American beech", "Beech", Species_common))

# Figure 1
fig1 <- sla_poster %>% 
  left_join(n_by_species, by = "Species_common") %>% 
  mutate(xlabel = paste(Species_common, n, sep = "\nN = ")) %>% 
  ggplot(aes(xlabel, specific_leaf_area)) +
  labs(title = "", x = "Species", y = expression(SLA~(cm^2~g^-1))) + # "SLA (cm2/g)") +
  geom_boxplot(aes(fill = Position)) + 
  scale_fill_manual(values = pal) 
print(fig1)
ggsave("poster_figures/fig1.png")
fig1alt <- plot_grid(fig1, ncol = 1, nrow = 1)
save_plot(filename = "poster_figures/fig1alt.png", plot = fig1alt)
...
print(fig2a)
ggsave("poster_figures/fig2a.png")

# For salinity, add a dummy tulip poplar row so a blank category is displayed
fig2b <- tibble(Species_common = "Tulip poplar", specific_leaf_area = NA, Salinity = NA) %>% 
  bind_rows(sla_shore_poster) %>% 
  ggplot(aes(Species_common, specific_leaf_area)) +
  labs(title = "", x = "Species", y = expression(SLA~(cm^2~g^-1))) + # "SLA (cm2/g)") +
  geom_boxplot(aes(fill = Salinity)) + 
  scale_fill_manual(values = pal)
print(fig2b)
ggsave("poster_figures/fig2b.png")

fig2 <- plot_grid(fig2a, fig2b, ncol = 1, nrow = 2)
print(fig2)
save_plot(filename = "poster_figures/fig2.png", plot = fig2)
```
...
labs(title = "Specific Leaf Area vs. Tree Diameter", 
y = "Specific Leaf Area (cm2/g)", 
x = "Diameter at Breast Height (cm)", color = "Species_common")
print(sla_vs_dbh)
```
...
labs(title = "Specific Leaf Area along Natural Salinity Gradient",
y = "Specific Leaf Area (cm2/g)") +
scale_fill_manual(values = pal)
...
ggplot(aes(Trap, annual_litter_mass, fill = Species_code)) +
geom_col() +
facet_wrap(~Plot) +
labs(title = "Leaf litterfal by plot and trap", y = "Leaf litterfall (g/m2)")
...
print()
```
Colors used for Line Graph 

# from Arches color palette
light_blue <- A8CDEC
orange <- F6955E
maroon <- 682C37
plum <- 9B6981
...
``` {r litter poster figure, echo = FALSE}

annual_litter <- litter_data_raw %>% 
  # filter for exactly one year of litter data
  mutate(Date_collected = mdy(Date_collected)) %>% 
  filter(Date_collected >= mdy("10/1/2018") & Date_collected < mdy("10/1/2019")) %>% 
  # restructure data, using only leaf litter
  ...
  # Traps are 0.5 m2; convert to 1 m2
  mutate(Value = Value * 2) %>% 
  # add all litter types together for each day of collection and each plot
  group_by(Date_collected, Plot) %>% 
  mutate(annual_litter = mean(Value))

# Line graph
avg_litter_plot <- annual_litter %>% 
  ggplot(aes(lubridate::yday(Date_collected), annual_litter, color = Plot)) +
  geom_line(size = 1.25) +
  scale_color_manual(
      values = c("HSHE" = "#A8CDEC", 
                 "HSLE" = "#F6955E", 
                 "HSME" = "#682C37", 
                 "LSHE" = "#9B6981", 
                 "LSLE" = "#7887A4", 
                 ...
                 "MSME" = "#969B4B")) +
  theme_classic() +
  labs(x = "Day of year", y = "Leaf litterfall (g/m2)")
      # title = "Annual Leaf Litterfall" can be used for labs() above 
print(avg_litter_plot)

lai_plot <- sla_litter_joined %>% 
# cm2/g * g/m2 = LAI; convert to m2/g for reporting 
mutate(LAI = avg_sla * annual_litter_mass / (100 * 100)) %>% 
group_by(Plot, Species_code) %>% 
summarise(lai_plot = round(sum(LAI, na.rm = TRUE), 2)) %>% 
ggplot(aes(Plot, lai_plot, fill = Species_code, color(value = pal))) +
geom_col() +
theme(axis.text.x = element_text(angle = 90), 
...
litter_lai_plot  <-
  ...
  draw_plot(lai_plot, x = 0.07, y = .45, width = .5, height = .5)
print(litter_lai_plot)

ggsave("poster_figures/litter_lai_plot.png")
```