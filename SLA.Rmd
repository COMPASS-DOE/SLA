...
species_position %>% 
  ...
  geom_errorbar(aes(ymin = specific_leaf_area - sd_sla, ymax = specific_leaf_area + sd_sla), alpha = 0.33) +
...
ranged from `r min(mc_lai_plot$LAI)` 
...
exposure, with values from `r mc_lai_sal$HSLE$LAI` `r mc_lai_sal$HSLE$ci` at the high-salinity site,
to `r mc_lai_sal$MSLE$LAI` `r mc_lai_sal$MSLE$ci`,
to `r mc_lai_sal$LSLE$LAI` `r mc_lai_sal$LSLE$ci` at the lowest-salinity plot. 
Spatial variability in leaf litter production contributed much more to the uncertainty in LAI 
estimates than did variability in the SLA measurements (**Figure 7**).
