...
# If no internet connectivity, sites_map will not exist, so skip
if(exists("sites_map")) {
  ggmap(sites_map) +
    geom_point(
      data = sites,
...
# Write out data for ESS-DIVE submission
sla %>% 
  mutate(Date = as.character(mdy(sla$Date)),
         SLA = round(SLA, 3)) %>% 
  select(-Notes.x, -Notes.y, -Plot, -Species, -Species_common) %>% 
  write_csv("./ess-dive/sla_data.csv", na = "-9999")
```
...
```{r TEMPEST}
sla %>%
  filter(Plot %in% c("Salt", "Fresh", "Control")) %>% 
  group_by(Species_code, Species) %>%
  summarise(
    n = n(),
    mean_SLA = mean(SLA) %>% round(1),
...
kable(table1, 
      digits = 1, format = "html",
      col.names = c("Species_code", "Species", "N", "SLA", "s.d."),
      caption = "Table X: Specific leaf area (cm2/g) by species for TEMPEST plots") %>%
  kable_styling(bootstrap_options = bo)
```
...
hsd_species$groups %>%
  ...
  separate(species, into = c("Species_code", "Position"), sep = "\\.") %>%
  left_join(xx, by = c("Species_code", "Position")) %>% 
  # There is no sun CAGL so add an NA row for it 
  complete(Species_code, Position) %>% 
  arrange(SLA) ->
species_position

species_position %>%
  ggplot(aes(Species_code, SLA, group = Position, fill = Position)) +
  geom_col(position = "dodge") +
  geom_errorbar(aes(ymin = SLA - sd_sla, ymax = SLA + sd_sla), alpha = 0.33, position = "dodge") +
  geom_text(aes(label = groups), vjust = -1, position = position_dodge(width = 0.9)) +
  xlab("Species") + ylab(expression(SLA ~ (cm^2 ~ g^-1))) +
  scale_fill_brewer() +
  theme(
...
litter_2019 %>%
  ...
  # make a pretty plot name
  mutate(salinity = substr(Plot, 1, 1),
         salinity = case_when(salinity == "H" ~ "High",
                              salinity == "M" ~ "Medium",
                              salinity == "L" ~ "Low"),
         Plot = factor(Plot, levels = c("LSLE", "LSME", "LSHE", "MSLE", "MSME", "MSHE", "HSLE", "HSME", "HSHE"))) %>% 
  # ...and plot
  ggplot(aes(yearday, cum_litter, fill = Species_code)) +
  geom_area(position = "stack") +
  facet_wrap(~Plot) +
  coord_cartesian(xlim = c(184, 366)) +
  geom_text(aes(label = salinity), x = 250, y = 200, size = 2) +
  theme(
...
ggsave("figures/fig_si-lai-mc.png")
