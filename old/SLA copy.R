...
leaves_v_area <- ggplot(data = sla, aes(n_Leaves, Leaf_Area_cm2, color = Date)) +
  geom_jitter() +
  labs(
    title = "Number of Leaves vs. Leaf Area",
...
sla_vs_dbh <- sla_joined_names %>%
  ...
  labs(
    title = "Specific Leaf Area vs. Tree Diameter", y = "Specific Leaf Area (cm2/g)", x = "Diameter at Breast Height (cm)",
    color = "Species"
  )
print(sla_vs_dbh)
